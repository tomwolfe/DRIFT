...
def test_dose_response():
    ...
    assert growth_rates[0] > growth_rates[1] * 0.99, f"Basal {growth_rates[0]} should be >= {growth_rates[1]}"
    assert growth_rates[1] > growth_rates[2], f"Growth {growth_rates[2]} should be < {growth_rates[1]}"
...
def test_signaling_delays():
    ...
    assert final_akt < 0.5, f"AKT should have dropped significantly, got {final_akt:.4f}"
    assert final_mtor < 0.6, f"mTOR should have dropped significantly, got {final_mtor:.4f}"
...
def test_metabolic_response():
    ...
    print(f"DEBUG: Basal growth={basal_growth:.4f}, Inhibited growth={inhibited_growth:.4f}")
