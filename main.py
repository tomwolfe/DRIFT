...
def main(config: SimulationConfig = None):
    ...
    try:
        ...
        print(f"Mean Treated Growth:    {avg_final_growth:.4f} h⁻¹ (±{std_final_growth:.4f})")
...
def parse_args():
    parser = argparse.ArgumentParser(description='DRIFT: Multi-Scale Stochastic Research Workbench')
    ...
    parser.add_argument('--mc-iterations', type=int, help='Number of Monte Carlo iterations')
    parser.add_argument('--model-name', type=str, help='Metabolic model name')
    parser.add_argument('--output', type=str, default='drift_dashboard.html', help='Output file name')
