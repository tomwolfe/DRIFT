...
def main():
    ...
    plt.fill_between(
        time_points, ci_lower, ci_upper, color="red", alpha=0.2, label="95% CI"
    )

    plt.axhline(
        y=basal_growth, color="black", linestyle="--", alpha=0.7, label="Basal Growth"
    )
    plt.xlabel("Time Steps")
    plt.ylabel("Growth Rate (hr⁻¹)")
    plt.title(
        f"Monte Carlo Analysis: {N_SIMULATIONS} Stochastic Simulations\nDrug Kd={DRUG_KD}, Conc={DRUG_CONCENTRATION}"
    )
    plt.legend()
