...
def main():
    ...
    for result in drug_results:
        plt.plot(
            result["growth"],
            label=f"{result['name']} (Kd={result['kd']})",
            color=result["color"],
            linewidth=2,
        )
    ...
    plt.title(
        "Comparative Drug Response: Effect of Binding Affinity on Metabolic Drift"
    )
    plt.legend()
    ...
    for result in drug_results:
        ...
        print(
            f"{result['name'][:19]:<20} {result['kd']:<8.2f} {final_growth:<12.4f} {inhibition_pct:<12.1f}"
        )
    ...
    closest_idx = min(
        range(len(growth_rates)), key=lambda i: abs(growth_rates[i] - half_max)
    )
