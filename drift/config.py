...
class SimulationConfig:
    ...
    def __post_init__(self):
        ...
        if self.drug_concentration < 0:
            raise ValueError(f"drug_concentration must be non-negative, got {self.drug_concentration}")
            
        if self.sim_steps <= 0:
            raise ValueError(f"sim_steps must be positive, got {self.sim_steps}")
            
        if self.mc_iterations <= 0:
            raise ValueError(f"mc_iterations must be positive, got {self.mc_iterations}")
